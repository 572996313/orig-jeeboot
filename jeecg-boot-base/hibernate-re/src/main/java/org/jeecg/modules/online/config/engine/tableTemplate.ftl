                ...
                <#else>
                    <property name="${attr.dbFieldName}"
                        ...
                        <column name="${attr.dbFieldName}" <#if dataType=='SQLSERVER' && attr.dbType?lower_case="string"> sql-type="nvarchar(${attr.dbLength})" </#if>
                                ...
                                <#-- update-begin---author:chenrui ---date:20240617  for：[TV360X-350]Oracle下日期/日期时间默认值设置---------- -->
                                <#if attr.dbDefaultVal?exists&&attr.dbDefaultVal!=''>
                                    <#if attr.dbType == 'Date' && dataType=='ORACLE'>
                                        default="to_date(${attr.dbDefaultVal},'yyyy-mm-dd')"
                                    <#elseif attr.dbType == 'Datetime' && dataType=='ORACLE'>
                                        default="to_date(${attr.dbDefaultVal},'yyyy-mm-dd hh24:mi:ss')"
                                    <#else>
                                        default="${attr.dbDefaultVal}"
                                    </#if>
                                </#if>
                                <#-- update-end---author:chenrui ---date:20240617  for：[TV360X-350]Oracle下日期/日期时间默认值设置---------- -->
